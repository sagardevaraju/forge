...
- **`artifacts/portfolio_optimization.json`, `artifacts/calibration.json`, `artifacts/treaty.json`, and `artifacts/regime/*.parquet` ARE tracked** (Phase 1 P2.0 / Phase 2 P2.2 / Phase 2 P2.17 / Phase 2 P2.3). Pages and tests read them at render/run time; never re-run the precompute in serverless. Regenerate locally with `python -m scripts.precompute_portfolio_optimization`, `python -m scripts.precompute_calibration`, `python -m scripts.precompute_treaty`, and `python -m ml.scenarios.regime --refresh`.
