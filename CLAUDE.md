...
- **`artifacts/portfolio_optimization.json`, `artifacts/calibration.json`, `artifacts/regime/*.parquet`, and `artifacts/hurdat2/*.parquet` ARE tracked** (Phase 1 P2.0 / Phase 2 P2.2 / P2.3 / P2.10). Pages and tests read them at render/run time; never re-run the precompute in serverless. Regenerate locally with `python -m scripts.precompute_portfolio_optimization`, `python -m scripts.precompute_calibration`, `python -m ml.scenarios.regime --refresh`, and `python -m ml.scenarios.hurdat2 --refresh`.
