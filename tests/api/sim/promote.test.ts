...
describe('POST /api/sim/[id]/promote', () => {
  test('flips promoted=1 and returns the in-process K=1000 distribution', async () => {
    ...
    // numpy.histogram(bins=30) → 30 bins whose counts sum to K.
    expect(body.histogram.counts.length).toBe(30);
    expect(body.histogram.counts.reduce((a: number, b: number) => a + b, 0)).toBe(1000);

    // TVaR-99 is the mean of the top-1% tail, so it never sits below p99.
    expect(body.summary.tvar99).toBeGreaterThanOrEqual(body.summary.p99);
