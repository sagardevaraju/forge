...
export function ImpactPanel({ impact }: ImpactPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-3 text-sm">
      <div className="text-xs uppercase tracking-wider text-zinc-500">Preview impact</div>
      <div className="flex justify-between items-center pb-2 border-b border-zinc-200">
        <span className="text-zinc-600">Est. gross loss</span>
        <span className="text-2xl font-semibold text-red-600 tabular-nums">{fmtUSD(impact.gross_loss_estimate)}</span>
      </div>
      ...
      {impact.top_cohorts.length > 0 && (
        <div className="mt-2">
          <div className="text-xs uppercase tracking-wider text-zinc-500 mb-1">Top cohorts</div>
          {impact.top_cohorts.map((c) => (
            <div key={c.cohort_id} className="flex justify-between text-xs py-0.5 text-zinc-700">
              <span>{c.cohort_id}</span>
              <span className="text-red-600 tabular-nums">{fmtUSD(c.loss)}</span>
            </div>
          ))}
        </div>
      )}
